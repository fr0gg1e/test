...
function Build-DnsQuery {
    ...
    foreach ($label in $Name.Split('.')) {
        $qname += [byte]$label.Length
        $qname += [System.Text.Encoding]::ASCII.GetBytes($label)
    }
    $qname += [byte]0x00
...
function Resolve-DoT {
    ...
    try {
        ...
        $query = Build-DnsQuery -Name $Name
        $lenPrefix = [byte[]]@(([math]::Floor($query.Length / 256)), ($query.Length % 256))
        $ssl.Write($lenPrefix + $query)
        $ssl.Flush()

        $respLen = [byte[]]::new(2)
        $ssl.Read($respLen, 0, 2) | Out-Null
        $totalLen = ([int]$respLen[0] -shl 8) + [int]$respLen[1]
        $respData = [byte[]]::new($totalLen)
        $read = 0
