...
int main(int ac, char **av) {
    ...
    snprintf(desc, sizeof(desc), "debug:t%d", getpid());
    errno = 0;
    r = syscall(__NR_request_key, "user", desc, "x", KEY_SPEC_SESSION_KEYRING);
...
int main(void) {
    ...
    errno = 0;
    rk = syscall(249, "user", "debug:probe", "x", -3);
    e1 = errno;
...
def main():
    ...
    if mode == "blocked":
        raise SystemExit(
            ...
            "Ensure keyutils is installed and unshare -Ur -m works."
        )
