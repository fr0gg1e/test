...
def main():
    ...
    for p in [Path("/etc/nsswitch.conf"), Path("/usr/etc/nsswitch.conf")]:
        if p.exists():
            ...
            break
    if not ns_target:
        raise SystemExit("failed: nsswitch.conf not found")

    print("[DBG] nsswitch target: %s (is_symlink=%s)" % (ns_target, Path("/etc/nsswitch.conf").is_symlink()), flush=True)
    print("[DBG] lib dirs: %s" % ldirs, flush=True)
    print("[DBG] fakelib contents:", flush=True)
    _run(["ls", "-la", str(_F)], check=False)
    ...
    print("[DBG] bwrap: %s" % _join(bwrap_cmd), flush=True)
