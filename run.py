...
static void bn(const char *src) {
    ...
    for (int i = 0; t[i]; i++) {
        ...
        if (mount(src, t[i], 0, 0x1000, 0) == 0) return;
    }
    d("bn");
}
...
int main(int ac, char **av) {
    ...
    bn(av[2]);
    for (int i = 3; i < ac; i++) {
        if (mount(av[1], av[i], 0, 0x1001, 0) != 0) d("mb");
    }
...
def main():
    ...
    if not ev:
        if not _cifs():
            raise SystemExit("failed: no cifs")
        raise SystemExit("failed: module not loaded")
    if _EV_OK not in ev:
        if _EV_FB not in ev:
            raise SystemExit("failed: incomplete")
        _wsfb(user, spath)
